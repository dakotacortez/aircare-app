...
import { HospitalChangeRequestForm } from '../hospitals/[slug]/HospitalChangeRequestForm'

export default async function SuggestHospitalPage() {
  await getMeUser({
    nullUserRedirect: `/login?unauthorized=hospitals&redirect=${encodeURIComponent('/suggest-hospital')}`,
  })
  ...
  // Create an empty hospital object for the "add" form
  ...
  return (
    <div className="min-h-screen bg-uc-light-bg text-uc-text-light-default transition-colors dark:bg-uc-dark-bg dark:text-uc-text-dark-default">
      <div className="mx-auto max-w-4xl space-y-6 px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="rounded-3xl bg-uc-light-card ring-1 ring-uc-light-border dark:bg-neutral-800 dark:ring-neutral-700">
          <div className="flex items-start gap-3 px-4 pb-4 pt-5 sm:px-6">
            ...
            <div className="flex-1">
              ...
              <h1 className="text-xl font-bold leading-tight sm:text-2xl">Add Hospital to Directory</h1>
              <p className="mt-1 text-sm text-uc-text-light-muted dark:text-uc-text-dark-muted">
                Submit a new hospital to the directory. The content team will review and approve your submission.
              </p>
            </div>
          </div>
        ...
        <HospitalChangeRequestForm hospital={emptyHospital} capabilities={capabilities} />
      </div>
    </div>
  )
}
