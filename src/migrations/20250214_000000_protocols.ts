import { MigrateDownArgs, MigrateUpArgs, sql } from '@payloadcms/db-postgres'

export async function up({ db }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
  -- Remove legacy protocol references and tables if they still exist
  ALTER TABLE IF EXISTS "payload_locked_documents_rels"
    DROP CONSTRAINT IF EXISTS "payload_locked_documents_rels_protocol_fk",
    DROP CONSTRAINT IF EXISTS "payload_locked_documents_rels_protocols_fk";

  DROP INDEX IF EXISTS "payload_locked_documents_rels_protocol_id_idx";
  DROP INDEX IF EXISTS "payload_locked_documents_rels_protocols_id_idx";

  ALTER TABLE IF EXISTS "payload_locked_documents_rels"
    DROP COLUMN IF EXISTS "protocol_id",
    DROP COLUMN IF EXISTS "protocols_id";

  ALTER TABLE IF EXISTS "protocol" DISABLE ROW LEVEL SECURITY;
  ALTER TABLE IF EXISTS "protocol_rels" DISABLE ROW LEVEL SECURITY;
  ALTER TABLE IF EXISTS "_protocol_v" DISABLE ROW LEVEL SECURITY;
  ALTER TABLE IF EXISTS "_protocol_v_rels" DISABLE ROW LEVEL SECURITY;
  ALTER TABLE IF EXISTS "protocols" DISABLE ROW LEVEL SECURITY;
  ALTER TABLE IF EXISTS "protocols_rels" DISABLE ROW LEVEL SECURITY;
  ALTER TABLE IF EXISTS "_protocols_v" DISABLE ROW LEVEL SECURITY;
  ALTER TABLE IF EXISTS "_protocols_v_rels" DISABLE ROW LEVEL SECURITY;

  DROP TABLE IF EXISTS "_protocols_v_rels" CASCADE;
  DROP TABLE IF EXISTS "_protocols_v" CASCADE;
  DROP TABLE IF EXISTS "protocols_rels" CASCADE;
  DROP TABLE IF EXISTS "protocols" CASCADE;
  DROP TABLE IF EXISTS "_protocol_v_rels" CASCADE;
  DROP TABLE IF EXISTS "_protocol_v" CASCADE;
  DROP TABLE IF EXISTS "protocol_rels" CASCADE;
  DROP TABLE IF EXISTS "protocol" CASCADE;

  DROP TYPE IF EXISTS "public"."enum_protocols_category" CASCADE;
  DROP TYPE IF EXISTS "public"."enum_protocols_status" CASCADE;
  DROP TYPE IF EXISTS "public"."enum__protocols_v_version_category" CASCADE;
  DROP TYPE IF EXISTS "public"."enum__protocols_v_version_status" CASCADE;
  DROP TYPE IF EXISTS "public"."enum_protocol_category" CASCADE;
  DROP TYPE IF EXISTS "public"."enum_protocol_status" CASCADE;
  DROP TYPE IF EXISTS "public"."enum__protocol_v_version_category" CASCADE;
  DROP TYPE IF EXISTS "public"."enum__protocol_v_version_status" CASCADE;

  -- Create fresh enums for the protocols collection
  CREATE TYPE "public"."enum_protocols_category" AS ENUM('medical', 'trauma', 'pediatric', 'neonatal', 'obgyn', 'procedures', 'behavioral', 'environmental', 'special-ops');
  CREATE TYPE "public"."enum_protocols_status" AS ENUM('draft', 'published');
  CREATE TYPE "public"."enum__protocols_v_version_category" AS ENUM('medical', 'trauma', 'pediatric', 'neonatal', 'obgyn', 'procedures', 'behavioral', 'environmental', 'special-ops');
  CREATE TYPE "public"."enum__protocols_v_version_status" AS ENUM('draft', 'published');

  -- Recreate the protocols tables using the schema produced by Payload
  CREATE TABLE "protocols" (
        "id" serial PRIMARY KEY NOT NULL,
        "_order" varchar,
        "title" varchar,
        "protocol_number" varchar,
        "category" "enum_protocols_category",
        "subcategory" varchar,
        "content_universal" jsonb,
        "content_b_l_s" jsonb,
        "content_a_l_s" jsonb,
        "content_c_c_t" jsonb,
        "special_considerations" jsonb,
        "key_points" jsonb,
        "references" jsonb,
        "effective_date" timestamp(3) with time zone,
        "last_reviewed" timestamp(3) with time zone,
        "next_review" timestamp(3) with time zone,
        "version_number" varchar,
        "keywords" varchar,
        "updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
        "created_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
        "_status" "enum_protocols_status" DEFAULT 'draft'
  );

  CREATE TABLE "protocols_rels" (
        "id" serial PRIMARY KEY NOT NULL,
        "order" integer,
        "parent_id" integer NOT NULL,
        "path" varchar NOT NULL,
        "media_id" integer
  );

  CREATE TABLE "_protocols_v" (
        "id" serial PRIMARY KEY NOT NULL,
        "parent_id" integer,
        "version__order" varchar,
        "version_title" varchar,
        "version_protocol_number" varchar,
        "version_category" "enum__protocols_v_version_category",
        "version_subcategory" varchar,
        "version_content_universal" jsonb,
        "version_content_b_l_s" jsonb,
        "version_content_a_l_s" jsonb,
        "version_content_c_c_t" jsonb,
        "version_special_considerations" jsonb,
        "version_key_points" jsonb,
        "version_references" jsonb,
        "version_effective_date" timestamp(3) with time zone,
        "version_last_reviewed" timestamp(3) with time zone,
        "version_next_review" timestamp(3) with time zone,
        "version_version_number" varchar,
        "version_keywords" varchar,
        "version_updated_at" timestamp(3) with time zone,
        "version_created_at" timestamp(3) with time zone,
        "version__status" "enum__protocols_v_version_status" DEFAULT 'draft',
        "created_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
        "updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
        "latest" boolean,
        "autosave" boolean
  );

  CREATE TABLE "_protocols_v_rels" (
        "id" serial PRIMARY KEY NOT NULL,
        "order" integer,
        "parent_id" integer NOT NULL,
        "path" varchar NOT NULL,
        "media_id" integer
  );

  ALTER TABLE "payload_locked_documents_rels" ADD COLUMN "protocols_id" integer;
  ALTER TABLE "protocols_rels" ADD CONSTRAINT "protocols_rels_parent_fk" FOREIGN KEY ("parent_id") REFERENCES "public"."protocols"("id") ON DELETE cascade ON UPDATE no action;
  ALTER TABLE "protocols_rels" ADD CONSTRAINT "protocols_rels_media_fk" FOREIGN KEY ("media_id") REFERENCES "public"."media"("id") ON DELETE cascade ON UPDATE no action;
  ALTER TABLE "_protocols_v" ADD CONSTRAINT "_protocols_v_parent_id_protocols_id_fk" FOREIGN KEY ("parent_id") REFERENCES "public"."protocols"("id") ON DELETE set null ON UPDATE no action;
  ALTER TABLE "_protocols_v_rels" ADD CONSTRAINT "_protocols_v_rels_parent_fk" FOREIGN KEY ("parent_id") REFERENCES "public"."_protocols_v"("id") ON DELETE cascade ON UPDATE no action;
  ALTER TABLE "_protocols_v_rels" ADD CONSTRAINT "_protocols_v_rels_media_fk" FOREIGN KEY ("media_id") REFERENCES "public"."media"("id") ON DELETE cascade ON UPDATE no action;
  ALTER TABLE "payload_locked_documents_rels" ADD CONSTRAINT "payload_locked_documents_rels_protocols_fk" FOREIGN KEY ("protocols_id") REFERENCES "public"."protocols"("id") ON DELETE cascade ON UPDATE no action;

  CREATE INDEX "protocols__order_idx" ON "protocols" USING btree ("_order");
  CREATE INDEX "protocols_updated_at_idx" ON "protocols" USING btree ("updated_at");
  CREATE INDEX "protocols_created_at_idx" ON "protocols" USING btree ("created_at");
  CREATE INDEX "protocols__status_idx" ON "protocols" USING btree ("_status");
  CREATE INDEX "protocols_rels_order_idx" ON "protocols_rels" USING btree ("order");
  CREATE INDEX "protocols_rels_parent_idx" ON "protocols_rels" USING btree ("parent_id");
  CREATE INDEX "protocols_rels_path_idx" ON "protocols_rels" USING btree ("path");
  CREATE INDEX "protocols_rels_media_id_idx" ON "protocols_rels" USING btree ("media_id");
  CREATE INDEX "_protocols_v_parent_idx" ON "_protocols_v" USING btree ("parent_id");
  CREATE INDEX "_protocols_v_version_version__order_idx" ON "_protocols_v" USING btree ("version__order");
  CREATE INDEX "_protocols_v_version_version_updated_at_idx" ON "_protocols_v" USING btree ("version_updated_at");
  CREATE INDEX "_protocols_v_version_version_created_at_idx" ON "_protocols_v" USING btree ("version_created_at");
  CREATE INDEX "_protocols_v_version_version__status_idx" ON "_protocols_v" USING btree ("version__status");
  CREATE INDEX "_protocols_v_created_at_idx" ON "_protocols_v" USING btree ("created_at");
  CREATE INDEX "_protocols_v_updated_at_idx" ON "_protocols_v" USING btree ("updated_at");
  CREATE INDEX "_protocols_v_latest_idx" ON "_protocols_v" USING btree ("latest");
  CREATE INDEX "_protocols_v_autosave_idx" ON "_protocols_v" USING btree ("autosave");
  CREATE INDEX "_protocols_v_rels_order_idx" ON "_protocols_v_rels" USING btree ("order");
  CREATE INDEX "_protocols_v_rels_parent_idx" ON "_protocols_v_rels" USING btree ("parent_id");
  CREATE INDEX "_protocols_v_rels_path_idx" ON "_protocols_v_rels" USING btree ("path");
  CREATE INDEX "_protocols_v_rels_media_id_idx" ON "_protocols_v_rels" USING btree ("media_id");
  CREATE INDEX "payload_locked_documents_rels_protocols_id_idx" ON "payload_locked_documents_rels" USING btree ("protocols_id");

  DROP TYPE IF EXISTS "public"."enum_protocol_category";
  DROP TYPE IF EXISTS "public"."enum_protocol_status";
  DROP TYPE IF EXISTS "public"."enum__protocol_v_version_category";
  DROP TYPE IF EXISTS "public"."enum__protocol_v_version_status";
  `)
}

export async function down({ db }: MigrateDownArgs): Promise<void> {
  await db.execute(sql`
  ALTER TABLE IF EXISTS "payload_locked_documents_rels"
    DROP CONSTRAINT IF EXISTS "payload_locked_documents_rels_protocols_fk";

  DROP INDEX IF EXISTS "protocols__order_idx";
  DROP INDEX IF EXISTS "protocols_updated_at_idx";
  DROP INDEX IF EXISTS "protocols_created_at_idx";
  DROP INDEX IF EXISTS "protocols__status_idx";
  DROP INDEX IF EXISTS "protocols_rels_order_idx";
  DROP INDEX IF EXISTS "protocols_rels_parent_idx";
  DROP INDEX IF EXISTS "protocols_rels_path_idx";
  DROP INDEX IF EXISTS "protocols_rels_media_id_idx";
  DROP INDEX IF EXISTS "_protocols_v_parent_idx";
  DROP INDEX IF EXISTS "_protocols_v_version_version__order_idx";
  DROP INDEX IF EXISTS "_protocols_v_version_version_updated_at_idx";
  DROP INDEX IF EXISTS "_protocols_v_version_version_created_at_idx";
  DROP INDEX IF EXISTS "_protocols_v_version_version__status_idx";
  DROP INDEX IF EXISTS "_protocols_v_created_at_idx";
  DROP INDEX IF EXISTS "_protocols_v_updated_at_idx";
  DROP INDEX IF EXISTS "_protocols_v_latest_idx";
  DROP INDEX IF EXISTS "_protocols_v_autosave_idx";
  DROP INDEX IF EXISTS "_protocols_v_rels_order_idx";
  DROP INDEX IF EXISTS "_protocols_v_rels_parent_idx";
  DROP INDEX IF EXISTS "_protocols_v_rels_path_idx";
  DROP INDEX IF EXISTS "_protocols_v_rels_media_id_idx";
  DROP INDEX IF EXISTS "payload_locked_documents_rels_protocols_id_idx";

  ALTER TABLE IF EXISTS "payload_locked_documents_rels" DROP COLUMN IF EXISTS "protocols_id";

  DROP TABLE IF EXISTS "protocols" CASCADE;
  DROP TABLE IF EXISTS "protocols_rels" CASCADE;
  DROP TABLE IF EXISTS "_protocols_v" CASCADE;
  DROP TABLE IF EXISTS "_protocols_v_rels" CASCADE;

  DROP TYPE IF EXISTS "public"."enum_protocols_category";
  DROP TYPE IF EXISTS "public"."enum_protocols_status";
  DROP TYPE IF EXISTS "public"."enum__protocols_v_version_category";
  DROP TYPE IF EXISTS "public"."enum__protocols_v_version_status";

  -- Restore the legacy protocol identifier column for payload locked documents
  ALTER TABLE IF EXISTS "payload_locked_documents_rels"
    ADD COLUMN IF NOT EXISTS "protocol_id" integer;

  CREATE INDEX IF NOT EXISTS "payload_locked_documents_rels_protocol_id_idx"
    ON "payload_locked_documents_rels" USING btree ("protocol_id");
  `)
}
